		...
		Route::namespace($namespace)->group(function () {
			Route::group(["prefix" => LaravelLocalization::setLocale(),"middleware" => [ "localeSessionRedirect", "localizationRedirect", "localeViewPath" ]], function(){
				Route::prefix("backend/orders")->group(function () {
                    ...
                    Route::post('/update-qty/{item}', 'OrdersController@updateQuantity')->name('update_orders_qty');
                    Route::get('/delete-order-item/{item}', 'OrdersController@deleteOrderItem')->name('delete_orders_item');
                    Route::get('/change-order-status/{order}/{status}', 'OrdersController@changeOrderStatus')->name('change_orders_status');
				});
			});
		});
	});
